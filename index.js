 ...
 *
 * Attributes:
 *  - Single-Page Application. Update entry.index to an array if you want to do
 *  a Multi-Page Application.
 *  - normalize.css is used. If this is not used, switch the loader include rule
 *  for CSS back to: exclude: /node_modules/
 *  - Pass the development / production mode flag via webpack CLI
 * */
...
module.exports = userConfig => {
  return (env, argv) => {
    ...
    return {
      ...
      entry: {
        index: path.join(userConfig.dirname, globals.ENTRY_PATH),
      },
      ...
      output: {
        path: path.join(userConfig.dirname, 'dist/static'),
      ...
      module: {
        rules: [
          ...
          // *** CSS ***
          {
            ...
            include: [
              path.resolve(userConfig.dirname, 'src'),
              path.resolve(userConfig.dirname, 'node_modules/normalize.css'),
            ],
          },
          ...
          // above the specified limit.
          {
            ...
            use: {
              loader: 'url-loader',
              options: {
                limit: globals.URL_LOADER_SIZE_LIMIT,
                name: devMode
                  ? 'assets/[name].[ext]'
          ...
          : [
              ...
              new HtmlWebpackPlugin({
                template: path.resolve(
                  userConfig.dirname,
                  globals.HTML_TEMPLATE_PROD_PATH,
                ),
      ...
      devServer: {
        ...
        contentBase: path.join(
          userConfig.dirname,
          globals.HTML_TEMPLATE_DEV_PATH,
        ),
      ...
      recordsPath: path.join(userConfig.dirname, 'webpackRecords.json'),
    };
  };
};
